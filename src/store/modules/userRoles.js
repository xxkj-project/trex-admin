...
import auth from '@/data/routerList'
  ...
  getInfo({ commit, state }) {
    ...
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        ...
        // auth must be a non-null array 路由表必须为非空数组
        if (!auth || auth.length === 0) {
          reject(new Error('getInfo: auth must be a non-null array!'))
        }
