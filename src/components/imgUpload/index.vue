 ...
 * @LastEditTime: 2021-01-07 17:58:04
 * @Desc: 图片上传 - 公共组件
-->
<template>
  <div class="uploadWrapper">
    ...
    >
      ...
      >
        <el-image :src="item" :preview-src-list="[item]" lazy></el-image>
...
export default {
  ...
  watch: {
    value: {
      handler(val) {
        // console.log('-watch-value-', this.value, '-watch-isFirstMount-', this.isFirstMount)
  ...
  methods: {
    ...
    syncElUpload(val) {
      ...
      console.log('同步el-upload数据', imgList)
      ...
      uploadImgOrVideo(formData)
        .then(res => {
          ...
          if (res.code === '200') {
            ...
            // console.log('-img-upload-url-', res.data[0])
            // TODO: 注意该域名为访问上传图片的地址，若上传图片后访问不到请向后台询问域名是否已更改 ==》 http://uchatsave.oss-cn-beijing.aliyuncs.com
            let imgUrl = `${imgHeadUrl}/${res.data[0]}`
    ...
    onRemoveHandler(index) {
      this.$confirm('确定删除该图片?', '提示', {
        ...
        .then(() => {
          ...
          console.log('-删除-', this.imgList, index)
        })
        .catch(() => {})
    },
    // 超限
