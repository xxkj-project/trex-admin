...
<template>
  <div class="menus-list-page">
    <el-button type="primary" icon="el-icon-plus" @click="onAddMenu">新增一级菜单</el-button>
    <div class="main">
      <div class="listBox">
        <div class="tableBox">
          <el-table :data="tableList" tooltip-effect="dark" style="width: 100%">
            ...
            <el-table-column label="操作" min-width="160">
              <template slot-scope="scope">
                ...
                <el-button type="primary" size="mini" icon="el-icon-view" @click="onCheck(scope.row, scope.$index)"
                  >查看二级菜单</el-button
                >
              </template>
            </el-table-column>
...
export default {
  ...
  methods: {
    onAddMenu() {
      this.dialogType = 'add'
      this.isAddEdit = true
      this.formData = { parentId: 0, menuName: '', perms: '' }
    },
    onDel(row) {
      console.log('-del-', row)
      this.$confirm('确认删除该一级菜单？', '提示', {
        type: 'warning',
        center: true
      })
        .then(() => {
    ...
    onCheck(row, index) {
      ...
      const { id, menuName } = row
      this.$router.push({ name: 'SubMenusList', query: { parentId: id, parentMenuName: menuName } })
    },
    handleSuccess() {
      this.getData()
    },
    getData() {
      ...
      getMenusList(params).then(res => {
        ...
        this.tableList = list
        // TODO:过滤掉承兑商信息菜单
        // this.tableList = list.filter(item => item.perms !== 'Acceptance')
      })
    }
  }
}
